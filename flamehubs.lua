...
print("3. Serviços obtidos, verificando LocalPlayer...")
...
print("4. LocalPlayer encontrado: " .. player.Name)
...
if not success or not playerGui then
    warn("❌ ERRO: PlayerGui não encontrado após 10 segundos! Erro: " .. tostring(err))
    return
end

print("5. PlayerGui encontrado")
...
local function parseGenerationNumber(text)
    print("📊 parseGenerationNumber chamado com texto: " .. tostring(text))
    if not text or type(text) ~= "string" then 
        print("⚠️ Texto inválido, retornando 0")
        return 0 
    end
    text = text:gsub('[%$%s]', '')
    ...
    local result = numberPart * multiplier
    print("📊 Resultado parse: " .. tostring(result))
    return result
end

local function applyAnimalHighlight(bestModel, displayName)
    print("🔦 applyAnimalHighlight chamado")
    print("🔦 bestModel: " .. tostring(bestModel))
    print("🔦 displayName: " .. tostring(displayName))
    
    if currentAnimalHighlight then 
        print("🔦 Removendo highlight anterior")
        currentAnimalHighlight:Destroy() 
        currentAnimalHighlight = nil 
    end
    if not bestModel or not displayName then 
        print("⚠️ applyAnimalHighlight: bestModel ou displayName é nil")
        return 
    end
    
    for _, child in pairs(bestModel:GetChildren()) do
        if child:IsA("Model") and child.Name == displayName then
            print("🔦 Modelo filho encontrado: " .. child.Name)
            ...
            if not hasValidParts then 
                print("⚠️ Nenhuma BasePart encontrada no modelo")
                return 
            end
            ...
            if not highlight.Adornee then
                print("🔦 Tentando encontrar PrimaryPart...")
            ...
            if highlight.Adornee then
                print("✅ Highlight aplicado com sucesso")
                ...
                print("❌ Não foi possível encontrar Adornee para o highlight")
                highlight:Destroy()
            end
        end
    end
    print("❌ Nenhum modelo correspondente encontrado")
end

local function createBillboard(basePart, generationText, displayName, rarityText, bestModel)
    print("📌 createBillboard chamado")
    print("📌 basePart: " .. tostring(basePart))
    print("📌 generationText: " .. tostring(generationText))
    print("📌 displayName: " .. tostring(displayName))
    
    if not basePart then 
        print("❌ createBillboard: basePart é nil")
        return 
    end
    ...
    for _, billboard in ipairs(currentTraitsBillboards) do 
        if billboard and billboard.Parent then 
            print("📌 Removendo billboard antigo")
            billboard:Destroy() 
        end 
    end
    ...
    print("✅ Billboard criado com sucesso")
    return billboard
end

local function checkForGoodPets()
    print("🔍 checkForGoodPets chamado")
    ...
    print("🔍 " .. count .. " labels Generation encontrados")
    print("🔍 Melhor valor: " .. bestValue)
    
    if bestValue >= 1000000 then
        foundGoodPets = true
        print("✅ Pets bons encontrados!")
        return true
    else
        foundGoodPets = false
        print("❌ Nenhum pet bom encontrado")
        return false
    end
end

local function updateGenerationESP()
    print("👁️ updateGenerationESP chamado")
    local bestValue = -math.huge
    local bestLabel = nil
    local bestModel = nil
    ...
    if not bestLabel then 
        print("👁️ Nenhum label Generation encontrado")
    ...
    print("👁️ Melhor label encontrado: " .. bestLabel.Text)
    
    local displayNameObj = bestLabel.Parent:FindFirstChild('DisplayName')
    if displayNameObj and string.find((displayNameObj.Text or ""):lower(), "fusing") then 
        print("👁️ DisplayName contém 'fusing', ignorando")
        return 
    end
    ...
    if not baseModel then 
        print("❌ Nenhum baseModel encontrado")
        return 
    end
    
    local basePart = baseModel:FindFirstChild('Base') or baseModel.PrimaryPart
    if not basePart then 
        print("❌ Nenhum basePart encontrado")
        return 
    end
    
    if currentBillboard then 
        print("👁️ Removendo billboard anterior")
        currentBillboard:Destroy() 
        currentBillboard = nil 
    end
    ...
    print("👁️ Criando novo billboard:")
    print("👁️ - DisplayName: " .. displayName)
    print("👁️ - Rarity: " .. rarityText)
    print("👁️ - Generation: " .. bestLabel.Text)
...
local function sendWebhook(tier, pet)
    print("🌐 sendWebhook chamado para tier: " .. tier)
    local webhookUrl = WEBHOOKS[tier]
    if not webhookUrl then 
        print("❌ Webhook URL não encontrada para tier: " .. tier)
        return 
    end
...
local function scanPets()
        ...
        if obj:IsA("TextLabel") and obj.Name == "Generation" then
            ...
            if not string.find((obj.Text or ""):lower(), "fusing") then
                ...
                if rarity == "Secret" or petName == "Strawberry Elephant" then
                    ...
                    print("🎯 Pet secreto encontrado: " .. petName .. " (" .. rarity .. ")")
                    local pet = {
                        name = petName,
                        value = value,
                        rarity = rarity,
    ...
    print("🔄 Scan completo: " .. count .. " pets verificados, " .. secretCount .. " secretos encontrados")
end
...
local function createGUI()
    ...
    local function createBtn(txt, callback)
        ...
        btn.MouseButton1Click:Connect(function()
            print("🖱️ Botão " .. txt .. " clicado, novo estado: " .. tostring(not state))
            state = not state
            if callback then
                callback(state)
            end
        ...
        update()
        print("✅ Botão " .. txt .. " criado com sucesso")
        return btn
    end

    -- Criar botões
    createBtn("Desync", function(v)
        print("🔧 Desync: " .. tostring(v))
        desyncOn = v
        if v then
            pcall(function() 
                StarterGui:SetCore("SendNotification", {
    ...
    createBtn("Invisible Base", function(v)
        print("🏠 Invisible Base: " .. tostring(v))
        baseOn = v
        local Plots = Workspace:FindFirstChild("Plots")
        if Plots then
            if v then
                print("🏠 Tornando base invisível")
            ...
            print("❌ Plots não encontrado no Workspace")
        end
    end)
    ...
    close.MouseButton1Click:Connect(function()
        print("📦 Minimizando GUI")
        main.Visible = false
        bar.Visible = true
    end)

    bar.MouseButton1Click:Connect(function()
        print("📦 Restaurando GUI")
        main.Visible = true
        bar.Visible = false
    end)
...
if not success then
    warn("❌ ERRO CRÍTICO ao criar GUI: " .. tostring(gui))
else
    print("✅ GUI criada com sucesso via pcall")
end

-- Loop do ESP
task.spawn(function()
    print("👁️ Iniciando loop do ESP...")
    while true do
        if espOn then
            local success, err = pcall(updateGenerationESP)
            if not success then
                warn("❌ Erro no updateGenerationESP: " .. tostring(err))
            end
        end
        task.wait(2)
    end
...
task.spawn(function()
    print("🔍 Iniciando loop de verificação de pets...")
    while true do
        if LocalPlayer and LocalPlayer.Name == "rafael_elterror" then
            local success, err = pcall(checkForGoodPets)
            if not success then
                warn("❌ Erro no checkForGoodPets: " .. tostring(err))
            end
        end
        task.wait(5)
    end
...
task.spawn(function()
    print("🔄 Iniciando loop de scan de pets...")
    while task.wait(5) do
        local success, err = pcall(scanPets)
        if not success then
            warn("❌ Erro no scanPets: " .. tostring(err))
        end
    end
end)
